...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        ar_block += [nn.Conv2d(in_channels=32,out_channels=8,kernel_size=3,stride=1,padding=1),
                    nn.ReLU(),
                    ]
        # size = (32, 8, 8)
        # size = (32, 16, 16)
        ...
        self.lstm = nn.LSTM(2048, 512, 1, batch_first=True)

        ar_fc_block += [#nn.Linear(in_features=1024,out_features=1024),
                    #nn.ReLU(),
        ...
        fc_block1 += [nn.Linear(in_features=128,out_features=6),
                    nn.Softmax(dim=1)
                    ]
    ...
    def forward(self, input, layers=[], encode_only=False):
        ...
        # self.maxpool2 = inception_v3.maxpool2
            ...
            ar_out = ar_out.view(-1, 16 * 16 * 8)
            
            ar_out = ar_out.unsqueeze(1)
            #print(ar_out.shape)
            ar_out, _ = self.lstm(ar_out)
            ar_out = ar_out.squeeze(1)
