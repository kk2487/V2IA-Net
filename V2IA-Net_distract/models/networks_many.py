...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        self.xception_conv1 = xception.conv1
        self.xception_bn1 = xception.bn1
        self.xception_act1 = xception.act1

        self.xception_conv2 = xception.conv2
        self.xception_bn2 = xception.bn2
        self.xception_act2 = xception.act2
        self.xception_pool2 = nn.MaxPool2d(kernel_size=2)
        ...
        ar_fc_block += [nn.Linear(in_features=10240,out_features=1024),
                    #nn.ReLU(),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                up_model += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
    ...
    def forward(self, input, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            ...
            output1 = self.inception_conv0(input)
            output1 = self.inception_conv1(output1)
            output1 = self.inception_conv2(output1)
            output1 = self.maxpool1(output1)
            #print(A_np.shape)

            output1 = self.inception_conv3(output1)
            output1 = self.padding(output1)
            output1 = self.inception_conv4(output1)
            feats.append(output1)
            #print("-----",output.shape)
            
            output2 = self.xception_conv1(input)
            output2 = self.xception_bn1(output2)
            output2 = self.xception_act1(output2)
    
            output2 = self.padding(output2)
            output2 = self.xception_conv2(output2)
            output2 = self.xception_bn2(output2)
            output2 = self.xception_act2(output2)
            output2 = self.xception_pool2(output2)
            feats.append(output2)

            feature = torch.cat((output1,output2), dim=1)

            #print("m-----",feature.shape)
            #output = self.conv1(output)
            #print("-----",output.shape)

            output_resnet = self.resnet(feature)

            feats.append(output_resnet)
            ...
            feature_fc = torch.cat((output3,ar_out), dim=1)

            #print("feature_fc ", feature_fc.shape)
            ar_out = self.ar_fc_block(feature_fc)

            output_ar = self.fc_block1(ar_out)
            output_dn = self.fc_block2(ar_out)

            ##-----------------------------------------------
            output = self.up_model(feature)
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        #self.att_conv1 = AugmentedConv(in_channels=dim, out_channels=dim, kernel_size=3, dk=40, dv=4, Nh=4, relative=True,stride=1, shape=64)        
