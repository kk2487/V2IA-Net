...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input, layers=[], encode_only=False):
        ...
        # self.maxpool2 = inception_v3.maxpool2
            ...
            output = self.padding(output)
            output = self.inception_conv4(output)
            #A_np = output.squeeze(0).cpu().numpy()*255
            #A_np = np.uint8(A_np)
            #print("-----",output.shape)
            output = self.cbam1(output)

            #B_np = output.squeeze(0).cpu().numpy()*255
            #B_np = np.uint8(B_np)
            
            #cv2.imshow("tmpA", cv2.resize(A_np[10], (320,320)))
            #cv2.imshow("tmpB", cv2.resize(B_np[10], (320,320)))
            #print(cv2.resize(A_np[10], (320,320)).shape)
            #cv2.imwrite("A.jpg", cv2.resize(A_np[10], (320,320)))
            #cv2.imwrite("B.jpg", cv2.resize(B_np[10], (320,320)))
