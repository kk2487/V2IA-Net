...
# 駕駛行為分析(動作預測) (使用distract/resnet_3d_old/ 內的程式)
    ...
    # opt.resume_path = './night_best.pth'
    # opt.model_depth = 50
...
# -------------------------------------------------------------------------------
    ...
    while(ret):
        ...
        v2ianet_model.set_input(data)  # unpack data from data loader
        v2ianet_model.test()           # run inference
        #visuals = v2ianet_model.get_current_visuals()  # get image results

        # 拿取DCL模型動作預測結果
        predict_gan = v2ianet_model.get_predict()['predict_A']
        #print(predict_gan)
        distract_output = classes[predict_gan.argmax()]
        #print("***", gan_predict_output)

        # 拿取DCL模型分心預測結果
        predict_dn = v2ianet_model.get_predict()['predict_A_dn']
        #print(predict_dn)
...
# 分心偵測
        ...
        print("\n")
        print("--------------處理時間--------------")
        print('DCL GAN      : ', round(cg_end-cg_start,3), '(s)')
...
# -------------------------------------------------------------------------------
        ...
        #cv2.putText(cg_im,"Fake Image",(30,30), font, 1,(0,0,255),2)
